(function() {
  ...
  _.each = function(collection, iterator) {
    //if collection is array, iterate over array and apply iterator function on each individual element
    ...
    //if object terate over object values and apply iterator function on each individual element
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if(!isSorted) {
      ...
      for(var i = 0; i < array.length; i++) { // array = [1, 2, 2, 3, 4, 4]
        if(_.indexOf(booleanArr, iterator(array[i])) === -1) { // booleanArr = []
          booleanArr.push(iterator(array[i])); // booleanArr = [t, f]
          result.push(array[i]); // result = [1, 2]
        }
      }
    }
    return result;
  ...
  _.reduce = function(collection, iterator, accumulator) {
    // if no accumulator set accumulator to collection[0]
    if(accumulator === undefined){
      accumulator = collection[0];
      collection = collection.slice(1);
    }
    // iterate over the collection array, apply the iterator to individual elements and assign to accumulator
  ...
  _.extend = function(obj) {
    var index;
    var extension = function(value, key) {
      obj[key] = value;
    };
    for(index = 1; index < arguments.length; index += 1) {
      _.each(arguments[index], extension);
    }
    return obj;
  };
  ...
  _.defaults = function(obj) {
    _.each(arguments, function(extentions) {
      for (var key in extentions) {
        if (obj[key] === undefined) {
          obj[key] = extentions[key];
        }
      };
    });
    return obj;
